...
namespace camerakit {
  ...
  static void CameraSurfaceTexture_updateTexImage(JNIEnv* env, jobject thiz, jfloatArray transformMatrix) {
      CameraSurfaceTexture* cameraSurfaceTexture = CameraSurfaceTexture_getCameraSurfaceTexture(env, thiz);
      jfloat* matrix = env->GetFloatArrayElements(transformMatrix, 0);
      cameraSurfaceTexture->updateTexImage(matrix);
  ...
  namespace jni {
    ...
    const char* const classPathName = "camerakit/android/preview/CameraSurfaceTexture";

    static JNINativeMethod methods[] = {
            {"nativeInit",           "(II)V", (void*) CameraSurfaceTexture_init},
            {"nativeSetSize",        "(II)V", (void*) CameraSurfaceTexture_setSize},
            {"nativeUpdateTexImage", "([F)V", (void*) CameraSurfaceTexture_updateTexImage},
